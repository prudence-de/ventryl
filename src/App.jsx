...
// Haptic patterns (navigator.vibrate — works on Android/supported browsers)
...
function RippleLayer({ ripples, color = "rgba(255,255,255,0.22)" }) {
  return ripples.map(r => (
    <span key={r.id} style={{
      ...
      animation:"vRipple 0.65s cubic-bezier(0.4,0,0.2,1) forwards",
      pointerEvents:"none", zIndex:10,
    }}/>
  ));
}

// Button with ripple + haptic + press animation
function Btn({
  children, onClick, style, disabled, hapticPattern,
  rippleColor, className, onMouseEnter, onMouseLeave, ...rest
}) {
  ...
  const handleClick = (e) => {
    if (disabled) return;
    addRipple(e);
    haptic(hapticPattern || H.tap);
    onClick?.(e);
  };

  const computedStyle = {
    position:"relative", overflow:"hidden",
    transform: pressed ? "scale(0.96)" : hovered ? "scale(1.01)" : "scale(1)",
    transition:"transform 0.12s cubic-bezier(0.4,0,0.2,1), background 0.15s, color 0.15s, border-color 0.15s, box-shadow 0.15s",
...
function MotionCard({ children, style, onClick, hapticPattern, lift = true }) {
  ...
  return (
    <div
      ...
      style={{
        position:"relative", overflow:"hidden",
        transform: pressed ? "scale(0.985)" : (lift && hovered) ? "translateY(-2px)" : "none",
        transition:"transform 0.15s cubic-bezier(0.4,0,0.2,1), box-shadow 0.15s",
        boxShadow: lift && hovered ? "0 4px 16px rgba(0,0,0,0.08)" : "none",
        cursor: onClick ? "pointer" : "default",
        ...style,
      }}
    >
      {children}
      {onClick && <RippleLayer ripples={ripples} color="rgba(0,0,0,0.05)" />}
    </div>
  );
}

// Animated page view wrapper
function PageView({ children, viewKey }) {
  return (
    <div key={viewKey} style={{ animation:"vFadeUp 0.28s cubic-bezier(0.4,0,0.2,1) both" }}>
      {children}
    </div>
  );
}
...
// Staggered list item
function StaggerItem({ children, index, style }) {
  return (
    <div style={{
      animation:`vFadeUp 0.3s cubic-bezier(0.4,0,0.2,1) both`,
...
const ChartTip = ({active,payload,label}) => {
  ...
  return (
    <div style={{background:T.black,padding:"10px 14px",borderRadius:"6px",fontFamily:F,animation:"vFadeIn 0.15s ease"}}>
...
function KpiCard({label,value,sub,alert,accent,index=0}) {
  return (
    <div style={{
      background:T.white, padding:"18px 20px",
      borderLeft:alert?`3px solid ${T.amber}`:accent?`3px solid ${accent}`:"none",
      animation:"vScaleIn 0.35s cubic-bezier(0.4,0,0.2,1) both",
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:T.black,borderTop:"1px solid #1A1A1A",display:"flex",zIndex:100,paddingBottom:"env(safe-area-inset-bottom)"}}>
        {navItems.map(n=>(
          <Btn
            key={n.id}
            hapticPattern={H.nav}
            rippleColor="rgba(6,193,103,0.2)"
            onClick={()=>setActive(n.id)}
            style={{
              flex:1, display:"flex", flexDirection:"column", alignItems:"center",
              justifyContent:"center", padding:"10px 4px", background:"none",
              border:"none", fontFamily:F, color:active===n.id?T.green:"#555",
              position:"relative", minHeight:"56px",
            }}
          >
            {n.badge&&(
              <span style={{
                position:"absolute", top:"6px", right:"calc(50% - 14px)",
                background:T.red, color:T.white, fontSize:"9px", fontWeight:800,
                padding:"1px 4px", borderRadius:"8px", minWidth:"16px", textAlign:"center",
                animation:"vPulse 2s ease-in-out infinite",
              }}>{n.badge}</span>
            )}
            <span style={{
              display:"block",
              transform: active===n.id ? "scale(1.1)" : "scale(1)",
            ...
            {active===n.id&&(
              <span style={{
                ...
                width:"16px", height:"2px", background:T.green, borderRadius:"1px",
    ...
    <div style={{width:"210px",background:T.black,minHeight:"100vh",display:"flex",flexDirection:"column",flexShrink:0,position:"sticky",top:0,height:"100vh",overflowY:"auto"}}>
      <div style={{padding:"24px 20px 20px",borderBottom:"1px solid #1A1A1A"}}>
        <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
          <div style={{
            width:"30px", height:"30px", background:T.green,
      ...
      <nav style={{padding:"14px 10px",flex:1}}>
        {navItems.map((n,i)=>(
          <Btn
            key={n.id}
            hapticPattern={H.nav}
            rippleColor={active===n.id?"rgba(0,0,0,0.06)":"rgba(255,255,255,0.1)"}
            onClick={()=>setActive(n.id)}
            style={{
              width:"100%", display:"flex", alignItems:"center", gap:"9px",
              padding:"10px 12px", borderRadius:"5px",
              background:active===n.id?T.white:"transparent",
              color:active===n.id?T.black:"#888",
              border:"none", marginBottom:"2px", fontFamily:F,
              fontSize:"12px", fontWeight:active===n.id?800:600, textAlign:"left",
              animationDelay:`${i * 0.05}s`,
              animation:"vSlideLeft 0.3s both",
            }}
          >
            <span style={{
              transition:"transform 0.2s cubic-bezier(0.34,1.56,0.64,1)",
            ...
            {n.badge&&(
              <span style={{
                background:T.red, color:T.white, fontSize:"10px", fontWeight:800,
    ...
    <div style={{background:T.white,borderBottom:`1px solid ${T.gray100}`,padding:`0 ${isMobile?"16px":"28px"}`,height:"52px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:50}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:"8px",flexWrap:"nowrap"}}>
        {pills?.filter((_,i)=>!isMobile||i<2).map((p,i)=>(
          <div key={i} style={{
            background:p.bg, color:p.color, fontSize:"10px", fontWeight:800,
            padding:"3px 8px", borderRadius:"3px", whiteSpace:"nowrap",
            animation:`vFadeIn 0.3s ${i*0.1}s both`,
          }}>{p.label}</div>
        ))}
      </div>
    </div>
...
function OrderFlow({onDone,isMobile}) {
  ...
  if (done) return (
    <div style={{maxWidth:"420px",margin:"32px auto",textAlign:"center",padding:"0 16px",animation:"vFadeUp 0.4s ease"}}>
      <div style={{
        width:"60px", height:"60px", background:T.greenLight, borderRadius:"50%",
        display:"flex", alignItems:"center", justifyContent:"center",
        margin:"0 auto 18px", fontSize:"24px",
        animation:"vCheckPop 0.5s cubic-bezier(0.34,1.56,0.64,1) 0.1s both",
      }}>✓</div>
      <div style={{fontSize:"20px",fontWeight:800,color:T.black,marginBottom:"6px",animation:"vFadeUp 0.3s 0.2s both"}}>Order Submitted</div>
      <div style={{fontSize:"13px",color:T.gray400,marginBottom:"24px",animation:"vFadeUp 0.3s 0.3s both"}}>{sel?.name} will confirm within 2 hours. Funds held in KrediBank escrow.</div>
      <div style={{border:`1px solid ${T.gray100}`,marginBottom:"18px",textAlign:"left",animation:"vFadeUp 0.3s 0.35s both"}}>
        {[["Order ID","VTL-00844"],["Depot",sel?.name],["Product",prod],["Volume",`${(vol/1000).toFixed(0)}k L`],["Trucks",`${trucks} tanker${trucks>1?"s":""}`],["Total (Escrow)",`₦${total.toLocaleString()}`]].map(([k,v],i)=>(
          ...
          <div key={s} style={{display:"flex",alignItems:"center",flex:i<arr.length-1?"1":"0"}}>
            <div style={{display:"flex",alignItems:"center",gap:"6px",whiteSpace:"nowrap"}}>
              <div style={{
                ...
                background:step>i+1?T.green:step===i+1?T.black:T.gray200,
                color:step>=i+1?T.white:T.gray400,
                display:"flex",alignItems:"center",justifyContent:"center",
                fontSize:"10px",fontWeight:800,flexShrink:0,
                transition:"all 0.3s cubic-bezier(0.34,1.56,0.64,1)",
                transform:step===i+1?"scale(1.1)":"scale(1)",
              }}>{step>i+1?"✓":i+1}</div>
      ...
      {step===1&&(
        <div style={{animation:"vFadeUp 0.25s ease"}}>
          ...
          {[...DEPOTS].sort((a,b)=>a.pms-b.pms).map((d,i)=>(
            <MotionCard
              key={d.id}
              hapticPattern={H.soft}
              onClick={()=>{ haptic(H.soft); setSel(d); }}
              style={{
                border:`2px solid ${sel?.id===d.id?T.green:T.gray100}`,
                background:T.white, padding:"16px", marginBottom:"10px",
                transition:"border-color 0.15s",
                animation:`vFadeUp 0.3s ${i*0.06}s both`,
              }}
            >
              <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:"12px"}}>
                <div style={{display:"flex",alignItems:"flex-start",gap:"12px"}}>
                  <div style={{
                    ...
                    background:i===0?T.green:T.gray100,
                    borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",
                    fontSize:"12px",fontWeight:800,color:i===0?T.white:T.gray600,flexShrink:0,marginTop:"2px",
          ...
          <Btn
            disabled={!sel}
            hapticPattern={H.double}
            onClick={()=>setStep(2)}
            style={{
              background:sel?T.black:T.gray200, color:sel?T.white:T.gray400,
              border:"none", padding:"14px", fontSize:"13px", fontWeight:800,
              fontFamily:F, width:"100%", marginTop:"8px",
            }}
          >
      ...
      {step===2&&(
        <div style={{animation:"vFadeUp 0.25s ease"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:"12px",marginBottom:"18px"}}>
            <div style={{animation:"vFadeUp 0.25s 0.05s both"}}>
              ...
              <div style={{display:"flex",gap:"8px"}}>
                {["PMS",sel.ago?"AGO":null].filter(Boolean).map(p=>(
                  <Btn
                    ...
                    style={{
                      flex:1, padding:"12px",
                      border:`2px solid ${prod===p?T.black:T.gray200}`,
                      background:prod===p?T.black:T.white,
                      color:prod===p?T.white:T.black,
                      fontFamily:F, fontSize:"14px", fontWeight:800, minHeight:"48px",
                    }}
                  >{p}</Btn>
                ))}
              </div>
            </div>
            <div style={{animation:"vFadeUp 0.25s 0.1s both"}}>
          ...
          <div style={{marginBottom:"18px",animation:"vFadeUp 0.25s 0.15s both"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:"8px"}}>
              ...
              <div style={{fontSize:"13px",fontWeight:800,color:T.black,transition:"all 0.2s"}}>{(vol/1000).toFixed(0)}k L · {trucks} truck{trucks>1?"s":""}</div>
            </div>
            <input type="range" min={33000} max={198000} step={33000} value={vol}
              onChange={e=>{ setVol(Number(e.target.value)); haptic(H.soft); }}
          ...
          <div style={{background:T.black,padding:"18px",marginBottom:"16px",animation:"vFadeUp 0.25s 0.2s both"}}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"12px"}}>
              {[["Trucks",trucks],["Volume",`${(vol/1000).toFixed(0)}k L`],["Total",`₦${(total/1e6).toFixed(1)}M`]].map(([l,v],i)=>(
                <div key={l} style={{animation:`vScaleIn 0.25s ${0.25+i*0.06}s both`}}>
                  <div style={{fontSize:"9px",fontWeight:700,color:T.gray400,textTransform:"uppercase",marginBottom:"3px"}}>{l}</div>
                  <div style={{fontSize:"20px",fontWeight:800,color:T.white,transition:"all 0.3s"}}>{v}</div>
                </div>
              ))}
            </div>
          ...
          <Btn hapticPattern={H.double} onClick={()=>setStep(3)} style={{background:T.green,color:T.white,border:"none",padding:"14px",fontSize:"13px",fontWeight:800,fontFamily:F,width:"100%",minHeight:"48px",animation:"vFadeUp 0.25s 0.3s both"}}>Review Order →</Btn>
        </div>
      )}
      ...
      {step===3&&(
        <div style={{animation:"vFadeUp 0.25s ease"}}>
          ...
          <div style={{border:`1px solid ${T.gray100}`,background:T.white,marginBottom:"12px"}}>
            {[["Depot",sel.name],["Product",prod],["Price",`₦${price}/L`],["Volume",`${(vol/1000).toFixed(0)},000 L`],["Trucks",`${trucks}×33k L`],["ETA",sel.eta]].map(([k,v],i)=>(
            ...
            <div style={{display:"flex",justifyContent:"space-between",padding:"14px 18px",fontSize:"15px",animation:"vFadeUp 0.2s 0.3s both"}}>
          ...
          <div style={{background:T.amberLight,padding:"12px 16px",marginBottom:"10px",fontSize:"12px",color:"#8A5C00",fontWeight:600,lineHeight:1.5,animation:"vFadeUp 0.2s 0.25s both"}}>
            💡 Funds held by KrediBank until you confirm delivery.
          </div>
          <div style={{background:T.greenLight,padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px",animation:"vFadeUp 0.2s 0.3s both"}}>
          ...
          <Btn
            hapticPattern={H.confirm}
            onClick={()=>setDone(true)}
            style={{background:T.green,color:T.white,border:"none",padding:"14px",fontSize:"14px",fontWeight:800,fontFamily:F,width:"100%",minHeight:"48px",animation:"vFadeUp 0.2s 0.35s both"}}
          >
  ...
  return (
    <div>
      <div style={{background:T.black,padding:isMobile?"18px 16px":"24px 28px",marginBottom:"14px",animation:"vFadeIn 0.3s ease"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:"12px",flexWrap:isMobile?"wrap":"nowrap"}}>
          <div>
            ...
            <div style={{fontSize:"11px",color:T.gray400,marginTop:"3px",animation:"vFadeUp 0.3s 0.1s both"}}>RC-1092843 · Lagos · KYB ✓</div>
          </div>
          <div style={{textAlign:isMobile?"left":"right",animation:"vScaleIn 0.4s 0.1s both"}}>
            ...
            <Btn hapticPattern={H.tap} onClick={()=>{}} style={{marginTop:"8px",background:T.green,color:T.black,border:"none",padding:"7px 14px",fontSize:"11px",fontWeight:800,fontFamily:F,minHeight:"36px"}}>+ Fund Wallet</Btn>
          </div>
        </div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4,1fr)",gap:"1px",background:T.gray100,border:`1px solid ${T.gray100}`,marginBottom:"14px"}}>
        {[{l:"Orders MTD",v:"7",sub:"3 delivered"},{l:"Volume",v:"363k L",sub:"₦280.5M"},{l:"Avg. Price",v:"₦795/L",sub:"PMS · Mar"},{l:"Credit (VCS)",v:"720",sub:"Silver tier"}].map((k,i)=>(
          <KpiCard key={k.l} label={k.l} value={k.v} sub={k.sub} index={i}/>
        ))}
      </div>
        ...
        <Card pad={false} style={{animation:"vFadeUp 0.35s 0.1s both"}}>
          ...
          {ORDERS.filter(o=>o.status!=="delivered").map((o,i,arr)=>(
            <StaggerItem key={o.id} index={i}>
              <div style={{padding:"13px 18px",borderBottom:i<arr.length-1?`1px solid ${T.gray100}`:"none"}}>
                ...
                <div style={{height:"4px",background:T.gray100,borderRadius:"2px",overflow:"hidden"}}>
                  <div style={{
                    ...
                    background:o.status==="in_transit"?T.blue:T.amber,
                    borderRadius:"2px",
                    animation:"vProgressFill 1s ease both",
      ...
      <Card pad={false} style={{animation:"vFadeUp 0.35s 0.25s both"}}>
        ...
        {isMobile?(
          ORDERS.map((o,i)=>(
            <StaggerItem key={o.id} index={i}>
              <div style={{padding:"14px 18px",borderBottom:i<ORDERS.length-1?`1px solid ${T.gray100}`:"none"}}>
          ...
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>{ORDERS.map((o,i)=>(
              <tr key={o.id} style={{borderBottom:i<ORDERS.length-1?`1px solid ${T.gray100}`:"none",animation:`vFadeUp 0.25s ${i*0.08}s both`}}>
          ...
          {isMobile?(
            <>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"10px"}}>
                ...
                <Btn hapticPattern={H.double} onClick={onOrder} style={{background:T.black,color:T.white,border:"none",padding:"9px 16px",fontSize:"12px",fontWeight:800,fontFamily:F,minHeight:"40px",flexShrink:0}}>Order →</Btn>
              </div>
            </>
          ):(
            <div style={{display:"flex",alignItems:"center",gap:"16px"}}>
              ...
              <div style={{display:"flex",gap:"16px",alignItems:"center"}}>
                ...
                <Btn hapticPattern={H.double} onClick={onOrder} style={{background:T.black,color:T.white,border:"none",padding:"9px 16px",fontSize:"12px",fontWeight:800,fontFamily:F,minHeight:"40px"}}>Order →</Btn>
              </div>
            </div>
          )}
        </MotionCard>
      ...
      <div style={{background:T.black,padding:isMobile?"18px 16px":"24px 28px",marginBottom:"14px",animation:"vFadeIn 0.3s ease"}}>
        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:"24px"}}>
          <div>
            ...
            <div style={{fontSize:isMobile?"28px":"36px",fontWeight:800,color:T.green,letterSpacing:"-0.02em",lineHeight:1,animation:"vScaleIn 0.4s 0.1s both"}}>₦25,830,000</div>
            <div style={{fontSize:"11px",color:T.gray400,marginTop:"5px",animation:"vFadeUp 0.3s 0.2s both"}}>₦91.6M in escrow · 2 orders</div>
            <div style={{display:"flex",gap:"10px",marginTop:"16px",animation:"vFadeUp 0.3s 0.25s both"}}>
              <Btn hapticPattern={H.double} onClick={()=>{}} style={{background:T.green,color:T.black,border:"none",padding:"9px 16px",fontSize:"12px",fontWeight:800,fontFamily:F,minHeight:"40px"}}>+ Fund</Btn>
              <Btn hapticPattern={H.tap} onClick={()=>{}} style={{background:"transparent",color:T.white,border:"1px solid #333",padding:"9px 16px",fontSize:"12px",fontWeight:800,fontFamily:F,minHeight:"40px"}}>Withdraw</Btn>
            </div>
          </div>
          <div style={{animation:"vFadeUp 0.3s 0.15s both"}}>
      ...
      <Card pad={false} style={{animation:"vFadeUp 0.3s 0.15s both"}}>
        ...
        {TXN.map((t,i)=>(
          <StaggerItem key={t.id} index={i}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"13px 18px",borderBottom:i<TXN.length-1?`1px solid ${T.gray100}`:"none",gap:"10px"}}>
              <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
                <div style={{
                  width:"32px",height:"32px",borderRadius:"50%",
                  background:t.type==="credit"?T.greenLight:t.type==="released"?T.blueLight:T.gray100,
                  display:"flex",alignItems:"center",justifyContent:"center",fontSize:"13px",flexShrink:0,
                  transition:"transform 0.2s",
                }}
                  onMouseEnter={e=>e.currentTarget.style.transform="scale(1.1)"}
                  onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}
                >{t.type==="credit"?"↓":t.type==="released"?"🔓":"↑"}</div>
    ...
    <div>
      <div style={{background:T.black,padding:isMobile?"18px 16px":"24px 28px",marginBottom:"14px",display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:"12px",flexWrap:isMobile?"wrap":"nowrap",animation:"vFadeIn 0.3s ease"}}>
        <div>
          ...
          <div style={{fontSize:"11px",color:T.gray400,marginTop:"3px",animation:"vFadeUp 0.3s 0.1s both"}}>Apapa, Lagos · NMDPRA: MDP/D/0042</div>
        </div>
        <div style={{textAlign:isMobile?"left":"right",animation:"vScaleIn 0.4s 0.1s both"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:col2,gap:"14px",marginBottom:"14px"}}>
        <Card style={{animation:"vFadeUp 0.35s 0.1s both"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px"}}>
            ...
            <Btn hapticPattern={H.tap} onClick={()=>setEditing(!editing)} style={{background:editing?T.green:T.black,color:T.white,border:"none",padding:"7px 14px",fontSize:"11px",fontWeight:800,fontFamily:F,minHeight:"36px"}}>{editing?"Save":"Edit"}</Btn>
          </div>
          {[{label:"PMS",val:pms,set:setPms},{label:"AGO",val:ago,set:setAgo}].map((p,i)=>(
            <div key={p.label} style={{marginBottom:"12px",paddingBottom:"12px",borderBottom:`1px solid ${T.gray100}`,animation:`vFadeUp 0.25s ${i*0.08}s both`}}>
              ...
              {editing?(
                <div style={{display:"flex",alignItems:"center",border:`2px solid ${T.black}`,transition:"border-color 0.15s"}}>
                  <span style={{padding:"9px 10px",fontSize:"12px",fontWeight:700,color:T.gray400,borderRight:`1px solid ${T.gray100}`}}>₦</span>
                  <input type="number" value={p.val} onChange={e=>{ p.set(Number(e.target.value)); haptic(H.soft); }} style={{flex:1,border:"none",padding:"9px 10px",fontSize:"16px",fontWeight:800,fontFamily:F,outline:"none",color:T.black,width:"100%"}}/>
                  <span style={{padding:"9px 10px",fontSize:"11px",fontWeight:600,color:T.gray400}}>/L</span>
                </div>
              ):(
                <div style={{background:T.gray50,padding:"10px 12px",fontSize:"18px",fontWeight:800,color:T.black,transition:"all 0.2s"}}>₦{p.val.toLocaleString()}/L</div>
              )}
            </div>
          ))}
          <div style={{background:T.greenLight,padding:"10px 12px",display:"flex",alignItems:"center",gap:"7px"}}>
            ...
            <span style={{fontSize:"11px",fontWeight:700,color:T.greenDark}}>Live on marketplace</span>
          </div>
        </Card>
        ...
        <Card style={{animation:"vFadeUp 0.35s 0.18s both"}}>
          ...
          {[{prod:"PMS",current:61200,cap:85000},{prod:"Total",current:61200,cap:85000}].map((s,i)=>(
            <div key={s.prod} style={{marginBottom:"16px",animation:`vFadeUp 0.25s ${i*0.1}s both`}}>
          ...
          <div style={{background:T.amberLight,padding:"11px 14px",marginTop:"8px",animation:"vFadeUp 0.3s 0.3s both"}}>
  ...
  return (
    <div>
      <div style={{marginBottom:"20px"}}>
        <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"12px",animation:"vFadeUp 0.25s ease"}}>
          <div style={{fontSize:"14px",fontWeight:800,color:T.black}}>Incoming Orders</div>
          <div style={{
            background:T.redLight,color:T.red,fontSize:"10px",fontWeight:800,
            padding:"2px 7px",borderRadius:"10px",
            animation:"vPulse 2s ease-in-out infinite",
          }}>2 require action</div>
        </div>
        {INCOMING.filter(o=>o.status==="pending").map((o,i)=>(
          <StaggerItem key={o.id} index={i}>
            <div style={{border:acted[o.id]?`2px solid ${acted[o.id]==="confirm"?T.green:T.red}`:`2px solid ${T.amber}`,background:T.white,marginBottom:"12px",transition:"border-color 0.3s"}}>
              <div style={{padding:"16px"}}>
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:"8px",gap:"10px",flexWrap:isMobile?"wrap":"nowrap"}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:"10px 20px",flexShrink:0}}>
                    {[["Product",o.product],["Volume",`${(o.vol/1000).toFixed(0)}k L`],["Trucks",o.trucks],["Value",`₦${(o.value/1e6).toFixed(1)}M`]].map(([l,v],j)=>(
              ...
              <div style={{borderTop:`1px solid ${T.gray100}`,padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:"8px"}}>
                ...
                {acted[o.id]?(
                  <div style={{fontSize:"12px",fontWeight:800,color:acted[o.id]==="confirm"?T.greenDark:T.red,animation:"vScaleIn 0.3s cubic-bezier(0.34,1.56,0.64,1)"}}>
                    {acted[o.id]==="confirm"?"✓ Confirmed":"✗ Rejected"}
                  </div>
                ):(
                  <div style={{display:"flex",gap:"8px"}}>
                    <Btn hapticPattern={H.warning} onClick={()=>setActed(a=>({...a,[o.id]:"reject"}))} style={{background:T.white,color:T.red,border:`1px solid ${T.red}`,padding:"8px 14px",fontSize:"11px",fontWeight:800,fontFamily:F,minHeight:"40px"}}>Reject</Btn>
                    <Btn hapticPattern={H.confirm} onClick={()=>setActed(a=>({...a,[o.id]:"confirm"}))} style={{background:T.green,color:T.white,border:"none",padding:"8px 14px",fontSize:"11px",fontWeight:800,fontFamily:F,minHeight:"40px"}}>Confirm →</Btn>
                  </div>
                )}
              </div>
            </div>
      ...
      <div style={{fontSize:"14px",fontWeight:800,color:T.black,marginBottom:"12px",animation:"vFadeUp 0.25s 0.15s both"}}>Active Dispatches</div>
      {[{id:"VTL-00841",buyer:"Chukwuma Fuels Ltd",product:"PMS",vol:90000,trucks:3,stage:"in_transit",driver:"Emeka Nwosu",truck:"LSD-481-KJ",departure:"08:45",eta:"13:30",progress:65},
        {id:"VTL-00839",buyer:"Silvergate Energy",product:"PMS",vol:132000,trucks:4,stage:"loading",driver:"Bayo Adeyemi",truck:"LSD-219-AB",departure:"11:00",eta:"16:00",progress:25}].map((o,i)=>(
        <StaggerItem key={o.id} index={i}>
          <Card pad={false}>
            <div style={{padding:"14px 16px"}}>
              ...
              <div style={{height:"5px",background:T.gray100,borderRadius:"3px",overflow:"hidden"}}>
                <div style={{height:"100%",width:`${o.progress}%`,background:o.stage==="in_transit"?T.blue:T.amber,borderRadius:"3px",animation:"vProgressFill 1.2s ease both",animationDelay:`${i*0.15+0.3}s`}}/>
              </div>
    ...
    <div>
      <SectionHead title="Loading Bay Schedule" sub="Tue 10 Mar 2026 · Apapa Depot"
        right={<div style={{display:"flex",gap:"6px"}}>
          {["← Prev","Next →"].map(b=><Btn key={b} hapticPattern={H.soft} onClick={()=>{}} style={{background:T.white,border:`1px solid ${T.gray200}`,color:T.black,padding:"6px 10px",fontSize:"11px",fontWeight:700,fontFamily:F,borderRadius:"4px",minHeight:"36px"}}>{b}</Btn>)}
        </div>}/>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px",marginBottom:"14px"}}>
        {["Bay 1","Bay 2"].map((bay,bi)=>{
          const bs=SLOTS.filter(s=>s.bay===bay),booked=bs.filter(s=>s.status!=="open").length;
          return (
            <Card key={bay} style={{animation:`vScaleIn 0.3s ${bi*0.1}s both`}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:"8px"}}>
      ...
      {isMobile?(
          ...
          {SLOTS.map((s,i)=>(
            <StaggerItem key={i} index={i}>
              <div style={{border:`1px solid ${T.gray100}`,background:s.status==="open"?`${T.green}08`:T.white,padding:"14px 16px",marginBottom:"8px"}}>
                ...
                {s.status==="open"&&<Btn hapticPattern={H.tap} onClick={()=>{}} style={{background:T.black,color:T.white,border:"none",padding:"7px 14px",fontSize:"11px",fontWeight:700,fontFamily:F,width:"100%",minHeight:"40px"}}>Assign Order</Btn>}
                {s.status==="loading"&&<Btn hapticPattern={H.double} onClick={()=>{}} style={{background:T.green,color:T.white,border:"none",padding:"7px 14px",fontSize:"11px",fontWeight:700,fontFamily:F,width:"100%",minHeight:"40px"}}>Mark Departed</Btn>}
              </div>
            </StaggerItem>
          ))}
        </div>
        ...
        <Card pad={false} style={{animation:"vFadeUp 0.3s 0.15s both"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>{SLOTS.map((s,i)=>(
              <tr key={i} style={{borderBottom:i<SLOTS.length-1?`1px solid ${T.gray100}`:"none",background:s.status==="open"?`${T.green}08`:T.white,animation:`vFadeUp 0.2s ${i*0.05}s both`,transition:"background 0.2s"}}>
                ...
                <td style={{padding:"13px 18px"}}>
                  {s.status==="open"?<Btn hapticPattern={H.tap} onClick={()=>{}} style={{background:T.black,color:T.white,border:"none",padding:"6px 12px",fontSize:"11px",fontWeight:700,fontFamily:F,minHeight:"36px"}}>Assign</Btn>
                  :s.status==="loading"?<Btn hapticPattern={H.double} onClick={()=>{}} style={{background:T.green,color:T.white,border:"none",padding:"6px 12px",fontSize:"11px",fontWeight:700,fontFamily:F,minHeight:"36px"}}>Departed</Btn>
      ...
      {isMobile?(
        BUYERS_DATA.map((b,i)=>(
          <StaggerItem key={b.name} index={i}>
            <MotionCard hapticPattern={H.soft} style={{border:`1px solid ${T.gray100}`,background:T.white,padding:"14px 16px",marginBottom:"8px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"8px"}}>
                <div>
        ...
        <Card pad={false} style={{animation:"vFadeUp 0.3s ease"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>{BUYERS_DATA.map((b,i)=>(
              <tr key={b.name} style={{borderBottom:i<BUYERS_DATA.length-1?`1px solid ${T.gray100}`:"none",animation:`vFadeUp 0.25s ${i*0.07}s both`,transition:"background 0.15s"}}
                onMouseEnter={e=>{ e.currentTarget.style.background=T.gray50; haptic(H.soft); }}
    ...
    orders:(
      <div style={{animation:"vFadeUp 0.25s ease"}}>
        ...
        {isMobile?(
          ORDERS.map((o,i)=>(
            <StaggerItem key={o.id} index={i}>
              <div style={{border:`1px solid ${T.gray100}`,background:T.white,padding:"14px 16px",marginBottom:"8px"}}>
          ...
          <Card pad={false}>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              ...
              <tbody>{ORDERS.map((o,i)=>(
                <tr key={o.id} style={{borderBottom:i<ORDERS.length-1?`1px solid ${T.gray100}`:"none",animation:`vFadeUp 0.2s ${i*0.07}s both`}}>
    ...
    <div style={{fontFamily:F,background:T.black,minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 16px"}}>
      ...
      <div style={{textAlign:"center",marginBottom:"40px",animation:"vFadeUp 0.5s ease"}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:"12px",marginBottom:"14px"}}>
          <div style={{
            width:"40px",height:"40px",background:T.green,
            display:"flex",alignItems:"center",justifyContent:"center",
            animation:"vBounce 2s ease-in-out infinite",
          }}><span style={{fontSize:"20px",fontWeight:800,color:T.black}}>V</span></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:"12px",width:"100%",maxWidth:"480px"}}>
        {[{r:"buyer",title:"Buyer Portal",sub:"Petrol stations, aviation & industrial buyers",icon:"⛽",accent:T.green},{r:"depot",title:"Depot Portal",sub:"Licensed petroleum depots & suppliers",icon:"🏭",accent:T.blue}].map((x,i)=>(
          <MotionCard
            key={x.r}
            hapticPattern={H.success}
            onClick={()=>handleRoleSelect(x.r)}
            style={{
              background:"#0A0A0A",border:`1px solid #1A1A1A`,padding:"24px 20px",
              minHeight:isMobile?"auto":"160px",
              animation:`vFadeUp 0.4s ${i*0.12}s both`,
          ...
          >
            ...
            <div style={{fontSize:"12px",fontWeight:800,color:x.accent,transition:"letter-spacing 0.2s"}}>Enter →</div>
          </MotionCard>
        ))}
      </div>
